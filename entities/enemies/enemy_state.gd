...
func _check_horizontal_movement(
    ...
    if param < 0:
        _enemy.velocity.x = lerpf(_enemy.velocity.x, -speed, _enemy.ACCEL)
        _enemy.flip = true
    elif param > 0:
        _enemy.velocity.x = lerpf(_enemy.velocity.x, speed, _enemy.ACCEL)
        _enemy.flip = false
    else:
        _enemy.velocity.x = lerpf(_enemy.velocity.x, 0.0, _enemy.ACCEL)
...
func _check_vertical_movement(
    ...
    if param < 0:
        _enemy.velocity.y = lerpf(_enemy.velocity.y, -speed, 1)
    elif param > 0:
        _enemy.velocity.y = lerpf(_enemy.velocity.y, speed, 1)
    else:
        _enemy.velocity.y = lerpf(_enemy.velocity.y, 0.0, 1)
        if idle_state.length() > 0:
            state_change.emit(self.name, idle_state)
